...
const modules = [
  ...
  { id: 'blue_badge', name: { fa: 'بلیو بج (کارت پارکینگ معلولیت)', en: 'Blue Badge', uk: 'Синій значок' }, iconPath: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
...
const PaymentGateway = ({ moduleId, onUnlockModule, onUnlockFullAccess, onBack, lang }: { moduleId: string, onUnlockModule: () => void, onUnlockFullAccess: () => void, onBack: () => void, lang: 'fa' | 'en' | 'uk' }) => {
  ...
  const details = useMemo(() => {
    const paymentDetails = {
        pip: {
            price: 29.99,
            fa: { title: "دستیار فرم PIP را باز کنید", features: ["تمام ۱۷ بخش فرم PIP را با راهنمایی هوش مصنوعی تکمیل کنید.", "پاسخ‌های دقیق و شخصی‌سازی شده برای شرایط خود تولید کنید.", "یک درخواست قوی و متقاعدکننده برای افزایش شانس موفقیت خود آماده کنید.", "شامل ۲ بار استفاده از چک‌کننده فرم."], uses: "شامل یک بار استفاده کامل از دستیار فرم." },
            en: { title: "Unlock the PIP Form Assistant", features: ["Complete all 17 sections of the PIP form with AI guidance.", "Generate detailed, personalized answers for your situation.", "Prepare a strong, persuasive application to maximize your chances of success.", "Includes 2 uses of the Form Checker."], uses: "Includes one-time full use of the form assistant." },
            uk: { title: "Розблокуйте асистента для форми PIP", features: ["Заповніть усі 17 розділів форми PIP за допомогою ШІ.", "Створюйте детальні, персоналізовані відповіді для вашої ситуації.", "Підготуйте сильну, переконливу заявку, щоб максимізувати ваші шанси на успіх.", "Включає 2 використання перевірки форм."], uses: "Включає одноразове повне використання асистента." }
        },
        form_checker: {
            price: 9.99,
  ...
  const handlePayClick = async (purchaseType: 'module' | 'full_access') => {
    setProcessing(true);
    const price = purchaseType === 'module' ? details.price : 89.99;
    console.log(`Simulating payment of £${price} for ${purchaseType}...`);
    setTimeout(() => {
  ...
  const t = {
    fa: { pay: 'پرداخت امن', redirecting: 'در حال انتقال...', back: 'بازگشت', fullAccessTitle: 'دسترسی کامل ۳ روزه', bestValue: 'بهترین ارزش', fullAccessFeatures: ['دسترسی نامحدود به تمام دستیارهای فرم.', 'استفاده نامحدود از چک‌کننده فرم.', 'پشتیبانی ویژه به مدت ۳ روز.'], or: 'یا' },
    en: { pay: 'Pay Securely', redirecting: 'Redirecting...', back: 'Back', fullAccessTitle: '3-Day Full Access Pass', bestValue: 'Best Value', fullAccessFeatures: ['Unlimited access to all form assistants.', 'Unlimited uses of the Form Checker.', 'Priority support for 3 days.'], or: 'OR' },
    uk: { pay: 'Безпечна оплата', redirecting: 'Перенаправлення...', back: 'Назад', fullAccessTitle: 'Повний доступ на 3 дні', bestValue: 'Найкраща ціна', fullAccessFeatures: ['Необмежений доступ до всіх асистентів форм.', 'Необмежене використання перевірки форм.', 'Пріоритетна підтримка протягом 3 днів.'], or: 'АБО' },
  }[lang];

  return (
    <div className="bg-white/95 rounded-xl shadow-2xl p-6 sm:p-8 text-slate-800 animate-fade-in w-full max-w-4xl mx-auto text-center relative">
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-stretch">
            ...
            <div className="flex flex-col border rounded-lg p-6 w-full lg:w-1/2 text-left">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center mb-6 mt-auto">
                ...
                <button 
                    onClick={() => handlePayClick('module')}
                    disabled={processing}
                    className="w-full bg-slate-600 hover:bg-slate-700 text-white font-bold py-3 px-6 rounded-lg transition-colors text-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center shadow-lg"
                >
                    {processing ? t.redirecting : `${t.pay} £${details.price.toFixed(2)}`}
                </button>
            </div>

            <div className="flex items-center justify-center text-slate-500 font-bold my-4 lg:my-0">{t.or}</div>

            {/* Option 2: Full Access */}
            <div className="flex flex-col border-2 border-blue-600 rounded-lg p-6 w-full lg:w-1/2 relative bg-blue-50 text-left">
                 <div className="absolute top-0 right-4 -mt-4 bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase">{t.bestValue}</div>
                 <div className="flex-grow">
                    <h2 className="text-2xl font-bold text-blue-800 mb-4">{t.fullAccessTitle}</h2>
                    <div className="space-y-3 mb-8">
                        {t.fullAccessFeatures.map((feature, i) => (
                          <p key={i} className="flex items-start"><svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${lang === 'fa' ? 'ml-2' : 'mr-2'} text-green-500 shrink-0 mt-0.5`} viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>{feature}</p>
                        ))}
                    </div>
                </div>
                <div className="bg-white border border-blue-200 rounded-lg p-4 text-center mb-6 mt-auto">
                    <p className="text-4xl font-extrabold text-slate-800">£89.99</p>
                </div>
                <button 
                    onClick={() => handlePayClick('full_access')}
                    disabled={processing}
                ...
                >
                    {processing ? t.redirecting : `${t.pay} £89.99`}
                </button>
            </div>
        </div>
    </div>
...
const checkFullAccess = (): boolean => {
    try {
        const fullAccessData = localStorage.getItem('fullAccess');
        if (!fullAccessData) return false;
        const { unlocked, expiry } = JSON.parse(fullAccessData);
...
const App = () => {
  ...
  useEffect(() => {
    ...
    try {
        ...
        setIsFullAccess(checkFullAccess());
    } catch (e) {
  ...
  const handleUnlockModule = (moduleId: string) => {
    ...
    if (moduleId === 'pip') {
        const currentCheckerUses = unlockedModules['form_checker']?.usesLeft || 0;
        newState['form_checker'] = { unlocked: true, usesLeft: currentCheckerUses + 2 };
    }
  ...
  const handleUnlockFullAccess = () => {
    ...
    const fullAccessData = { unlocked: true, expiry };
    localStorage.setItem('fullAccess', JSON.stringify(fullAccessData));
  ...
  const renderContent = () => {
    if (selectedModuleId) {
      const isModuleUnlocked = !!unlockedModules[selectedModuleId]?.unlocked || isFullAccess;

      if (!isModuleUnlocked) {
        return (
          <PaymentGateway 
            moduleId={selectedModuleId} 
            onUnlockModule={() => handleUnlockModule(selectedModuleId)} 
            onUnlockFullAccess={handleUnlockFullAccess}
            onBack={handleBack} 
            lang={language} 
          />
        );
